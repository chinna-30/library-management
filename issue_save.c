#include"header.h"
void issue_save(struct isu *ptr)
{
struct isu *temp = ptr;

    FILE *fp = fopen("isuload.txt","w");

    while(temp != NULL)
    {
	    fprintf(fp,"%d|%d|%d|%s|%s|%s|%s|%d\n",temp->isid,temp->bid,temp->uid,
			    temp->uname,temp->isu_date,temp->due_date,temp->ret_date,temp->fine);
        temp = temp->next;
    }

    fclose(fp);
    fp = fopen("isubook.txt","w");
    ...
    printf("\n ISSUED FILE SAVED SUCCESSFULLY\n");
